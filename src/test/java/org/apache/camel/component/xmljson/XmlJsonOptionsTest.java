...
public class XmlJsonOptionsTest extends CamelTestSupport {
    ...
    public void testSomeOptionsToXML() throws Exception {
        ...
        // with expandable properties, array elements are converted to XML as a sequence of repetitive XML elements with the local name equal to the JSON key
        // for example: { number: [1,2,3] }, normally converted to: <number><e>1</e><e>2</e><e>3</e></number> (where e can be modified by setting elementName)
        // would be converted to <number>1</number><number>2</number><number>3</number>, if number is set as an expandable property
    ...
    public void testNamespacesDropped() throws Exception {
    	InputStream inStream = getClass().getClassLoader().getResourceAsStream("org/apache/camel/component/xmljson/testMessage2-namespaces.xml");
    	String in = context.getTypeConverter().convertTo(String.class, inStream);
        ...
        assertEquals("JSON must contain 1 top-level element", 1, obj.entrySet().size());
        assertTrue("Top-level element must be named root", obj.has("root"));
        // check that no child of the top-level element has a colon in its key, which would denote that 
    ...
    public void testCustomNamespaceMappings() throws Exception {
    	InputStream inStream = getClass().getClassLoader().getResourceAsStream("org/apache/camel/component/xmljson/testMessage2-namespaces.json");
    	String in = context.getTypeConverter().convertTo(String.class, inStream);
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        return new RouteBuilder() {
            ...
            public void configure() throws Exception {
            	XmlJsonDataFormatConfig config = new XmlJsonDataFormatConfig();
            	config.setEncoding("UTF-8");
            	config.setForceTopLevelObject(true);
            	config.setTrimSpaces(true);
            	config.setRootName("newRoot");
            	config.setSkipNamespaces(true);
            	config.setRemoveNamespacePrefixes(true);
            	config.setExpandableProperties(Arrays.asList("d", "e"));
                XmlJsonDataFormat format = new XmlJsonDataFormat(config);
            	...
            	XmlJsonDataFormatConfig namespacesConfig = new XmlJsonDataFormatConfig();
            	List<XmlJsonDataFormatConfig.NamespacesPerElementMapping> namespaces = new ArrayList<XmlJsonDataFormatConfig.NamespacesPerElementMapping>();
            	namespaces.add(new XmlJsonDataFormatConfig.NamespacesPerElementMapping("", "|ns1|http://camel.apache.org/test1||http://camel.apache.org/default|"));
            	namespaces.add(new XmlJsonDataFormatConfig.NamespacesPerElementMapping("surname", "|ns2|http://camel.apache.org/personalData|ns3|http://camel.apache.org/personalData2|"));
            	namespacesConfig.setNamespaceMappings(namespaces);
            	namespacesConfig.setTrimSpaces(true);
            	XmlJsonDataFormat namespacesFormat = new XmlJsonDataFormat(namespacesConfig);
            	// from XML to JSON
